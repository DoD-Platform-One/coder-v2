...
| coder.image | object | `{"pullPolicy":"IfNotPresent","pullSecrets":[],"repo":"registry1.dso.mil/ironbank/coder/coder-enterprise/coder-service-2","tag":"0.23.5"}` | The image to use for Coder. |
| coder.image.repo | string | `"registry1.dso.mil/ironbank/coder/coder-enterprise/coder-service-2"` | The repository of the image. |
| coder.image.tag | string | `"0.23.5"` | The tag of the image, defaults to {{.Chart.AppVersion}} if not set. If you're using the chart directly from git, the default app version will not work and you'll need to set this value. The helm chart helpfully fails quickly in this case. |
